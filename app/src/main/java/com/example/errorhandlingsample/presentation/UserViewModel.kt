...
import com.example.errorhandlingsample.domain.UserDataValidator
...
class UserViewModel(
    ...
    fun onRegisterClick(password: String) {
        when (val result = userDataValidator.validatePassword(password)) {
            ...
            is Result.Success -> {
                // do sth with the success response
            }
        }
        ...
        viewModelScope.launch {
            when (val result = repository.register(password)) {
                is Result.Error -> {
                    ...
                    // print this error out
                    _eventChannel.send(UserEvent.Error(errorMessage))
                }
